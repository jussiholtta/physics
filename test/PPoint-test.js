...
describe("physicsApp.Point", function() {
    ...
    it("matches equal points", function() {
      expect(point).toEqual(point2);
      });

    it("doesn't match unequal points", function() {
        point2.x = 14;
        expect(point).not.toEqual(point2);
        });

    it("is knows points in same spot have 0 distance between them", function() {
        expect(point.distance(point2)).toEqual(0);
        });

    it("is able to calculate distance", function() {
        point2.x = 2
        point2.y = 3
        point2.z = 4
    ...
    it("is able to move a point by adding a vector", function() {
        expect(point.plus(vector)).toEqual(new physicsApp.Point(2,4,6));
        });

    it("doesn't allow adding points together", function() {
